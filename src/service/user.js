...
module.exports = {
  getUserInfo: (ctx) =>
    new Promise((resolve) => {
      ...
      USER.findOne(
        ...
        'id email password verified playername uuid tokens skin isBanned ip time',
        { lean: true },
        (err, user) => {
          if (err) {
            resolve({ isLoggedIn: false })
          ...
          // 异地登陆需要重新登录
          if (
            ctx.session.userInfo.uuid === user.uuid &&
            ctx.session.userInfo.time.lastLogged !== user.time.lastLogged
          ) {
            ctx.session.userInfo = null
            resolve({ isLoggedIn: false })
            return
  ...
  changeUserVerifiedStatusById: (playerId, status) =>
    new Promise((resolve) => {
      ...
      USER.findOne({ email }, '', (err, user) => {
        if (!err && user) {
          ...
          user.save((e) => {
            ...
            // 检查旧hash的皮肤是否有人使用
            module.exports.searchUserInfoBySkinHash(oldSkinHash).then((result) => {
              // 无人使用，删除旧皮肤
              if (!result) {
  ...
  letUserLoggedIn: (ctx, email) =>
    new Promise((resolve) => {
      USER.findOne(
        { email },
        'id email playername verified password uuid tokens skin isBanned ip time',
        (err, user) => {
          if (err) throw err
  ...
  getUserSkin: (email) =>
    new Promise((resolve) => {
      USER.findOne({ email }, 'skin', { lean: true }, (err, user) => {
        ...
        resolve({ skinType, skin: `${skinHash}.png` })
      })
    }),
  ...
  genUserList: (filter, currectPage, pageSize = 8) =>
    new Promise((resolve) => {
      USER.countDocuments(filter || {}, (error, count) => {
        if (error) {
          resolve({ code: -1 })
        } else if (filter) {
          USER.find(filter, 'id email playername uuid isBanned time.register', { lean: true })
          ...
          USER.find(
            { id: { $gt: (currectPage - 1) * pageSize, $lte: currectPage * pageSize } },
            'id email playername uuid isBanned time.register',
            { lean: true }
          )
            .sort({ 'time.register': 1 })
            .exec((err, doc) => {
