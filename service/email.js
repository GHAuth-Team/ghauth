...
let transporter = nodemailer.createTransport(config.extra.smtp);
...
module.exports = {
    storeVerifyTokenToRedis: (playerId, token) => new Promise((resolve) => {
        ...
        let time = 5 * 60;
        // 储存对应玩家的邮箱验证Token
        let storeToken = redis.set(`verifyemail_${playerId}`, token, 'EX', time);
        // 储存对应玩家的邮箱验证Token的到期时间戳(10位)，过期时间5分钟
        let storeTokenTime = redis.set(`verifyemailtime_${playerId}`, Math.floor(new Date().getTime() / 1000) + time, 'EX', time);
    ...
    delVerifyToken: (playerId) => new Promise((resolve) => {
        let storeToken = redis.del(`verifyemail_${playerId}`);
        let storeTokenTime = redis.del(`verifyemailtime_${playerId}`);
    ...
    getVerifyTokenTime: (playerId) => new Promise((resolve) => {
        redis.get(`verifyemailtime_${playerId}`, (err, response) => {
            ...
            const result = new Date(parseInt(response)).getTime();
            resolve(result);
        })
    }),
